...
# --- Page Configuration ---
st.set_page_config(
    page_title="Web Termsly | Risk Analysis Dashboard",
    page_icon="Gemini_Generated_Image_fonq4qfonq4qfonq.png",
    layout="wide",
...
with st.sidebar:
    st.image("Gemini_Generated_Image_fonq4qfonq4qfonq.png", width=150) # Placeholder icon
    st.markdown("<h1 style='font-size: 25px; margin-bottom: 0px;'>Web Termsly AI</h1>", unsafe_allow_html=True)
    st.markdown("---")
...
if not analyze_btn and 'results' not in st.session_state:
    # Welcome / Empty State
    st.subheader("Welcome to Web Termsly (A Website Risk Analyzer AI)")
    ...
    if results:
        ...
        if dominant_risk == 'High Risk':
            ...
            st.warning(f"⚠️ **OVERALL STATUS: MEDIUM RISK** (Majority of clauses are medium risk)")
        else:
        ...
        with col1:
            ...
            st.download_button(
                ...
                data=pdf_data,
                file_name=f"Termsly_{url_input}.pdf",
